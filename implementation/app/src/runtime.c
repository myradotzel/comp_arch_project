...
/* These are for the non-volatile data logging*/
...
extern void checkpoint2(void);
extern void log_helper(uint8_t *data_size_base, uint8_t *data_dest_base, uint8_t *data_src_base, unsigned nrb);
extern void memcpy_helper(uint8_t* loc_data_dest, uint8_t* loc_data_src, uint8_t* loc_data_size);
...
/*TODO: add in __nv section to the linker file*/
...
const __stack = 0x1C00 + 0x1000; //should be the lowest addr in the stack;

/* For the non-volatile logging component of the runtime. comment out for now */
/**
...
//thereby committing this atomic region
...
void log_helper(uint8_t *data_size_base, uint8_t *data_dest_base, uint8_t *data_src_base, unsigned nrb) {
  *(data_size_base + nrb) = var_size;
  *(data_dest_base + nrb) = orig_addr;
  *(data_src_base + nrb) = backup_addr;
}

void memcpy_helper(uint8_t* loc_data_dest, uint8_t* loc_data_src, uint8_t* loc_data_size) {
  memcpy(loc_data_dest, loc_data_src, loc_data_size);
}
...
void checkpoint2() {
  ...
  */
  ...
  if (/*curctx->curr_m==Atomic||*/true_first) {
    __asm__ volatile ("MOV.W 0(R1), &0xFB80");
  ...
  __asm__ volatile ("MOV.W R4, &0xFB90");
  __asm__ volatile ("MOV.W R5, &0xFB94");
  __asm__ volatile ("MOV.W R6, &0xFB98");
  __asm__ volatile ("MOV.W R7, &0xFB9c");

  __asm__ volatile ("MOV.W R8, &0xFBA0");
  __asm__ volatile ("MOV.W R9, &0xFBA4");
  __asm__ volatile ("MOV.W R10, &0xFBA8");
  __asm__ volatile ("MOV.W R11, &0xFBAc");

  __asm__ volatile ("MOV.W R12, &0xFBB0");
  __asm__ volatile ("MOV.W R13, &0xFBB4");
  __asm__ volatile ("MOV.W R14, &0xFBB8");
  __asm__ volatile ("MOV.W R15, &0xFBBc");
...
void restore_vol() {
  ...
  */
  ...
  //__asm__ volatile ("MOV.W R1, &0xFBC0");

  __asm__ volatile ("MOV.W &0xFB84, R1");
  ...
  */
  ...
  i = 0;
  #endif
  __asm__ volatile ("MOV.W &0xFB90, R4");
  __asm__ volatile ("MOV.W &0xFB94, R5");
  __asm__ volatile ("MOV.W &0xFB98, R6");
  __asm__ volatile ("MOV.W &0xFB9c, R7");
  ...
  i = 0;
  #endif
  __asm__ volatile ("MOV.W &0xFBB0, R12");
  __asm__ volatile ("MOV.W &0xFBB4, R13");
  __asm__ volatile ("MOV.W &0xFBB8, R14");
  __asm__ volatile ("MOV.W &0xFBBc, R15");
  ...
  */
  ...
  __asm__ volatile ("MOV.W &0xFBA0, R8");
  __asm__ volatile ("MOV.W &0xFBA4, R9");
  __asm__ volatile ("MOV.W &0xFBA8, R10");
  __asm__ volatile ("MOV.W &0xFBAc, R11");
  ...
  */
  ...
  __asm__ volatile ("MOV.W &0xFB88, R2");
  __asm__ volatile ("MOV.W &0xFB84, R1");
  // /*
#ifndef LIBCAPYBARA_CONT_POWER
...
#endif
  //*/
  __asm__ volatile ("MOV.W &0xFB80, R0");
...
#endif
  unsigned int* save_point = (unsigned int*) 0xFBC8;
...
//Milijana: let's really try to have the proper entry function live in rustland.
/*
void entry() {
